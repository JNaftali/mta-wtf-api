...
Zip::File.open_buffer(Net::HTTP.get(URI('http://web.mta.info/developers/data/nyct/subway/google_transit.zip'))) do |zipfile|
  ...
  stops_table.each do |row|
    stop = Stop.find_or_create_by(mta_id: row[:stop_id])
    stop.assign_attributes(name: row[:stop_name], lat: row[:stop_lat], lon: row[:stop_lon])
    stop.parent_station = Stop.find_by(mta_id: row[:parent_station]) if row[:parent_station]
    stop.save
  end
  ...
  stop_times_table.each_with_index do |row, num|
    ...
    trip = Trip.find_by(mta_id: row[:trip_id])
    stop = Stop.find_by(mta_id: row[:stop_id])
    stop_time = StopTime.find_or_create_by(trip: trip, stop: stop)
